    ...
    <section v-if="page.demo" id="demo" class="mx-auto max-w-6xl px-6 py-10 sm:px-10">
      <div class="rounded-lg border border-dopa-border bg-dopa-panel p-6 shadow-glow">
        ...
        <div class="mt-6 grid gap-4 lg:grid-cols-3">
          <article v-for="step in page.demo.steps" :key="step.key" class="rounded-lg border border-dopa-border bg-dopa-card p-4">
            ...
            <div class="mt-3 flex flex-wrap gap-2">
              <span v-for="option in step.options" :key="option.value" class="rounded-full bg-dopa-bg px-3 py-1 text-xs text-dopa-muted">
                {{ option.label }}
              </span>
            </div>
          </article>
        ...
        <div class="mt-6 flex flex-wrap gap-2">
          <span v-for="label in page.demo.resultLabels" :key="label" class="rounded-md border border-dopa-green/40 px-3 py-2 text-xs text-dopa-text">
            {{ label }}
          </span>
        </div>
      </div>
