...
const ctaClass = (variant = 'primary') => {
  if (variant === 'secondary') {
    return 'border border-dopa-amber/50 bg-dopa-amber/10 text-dopa-text hover:bg-dopa-amber/20'
  }

  if (variant === 'ghost') {
    return 'border border-dopa-border bg-transparent text-dopa-muted hover:border-dopa-amber/60 hover:text-dopa-text'
  }

  return 'bg-dopa-amber text-dopa-bg hover:bg-white'
}
</script>
...
<template>
  <main class="min-h-screen bg-dopa-bg text-dopa-text">
    <section class="mx-auto max-w-6xl px-6 py-20 sm:px-10">
      <p class="text-sm font-semibold uppercase tracking-[0.24em] text-dopa-amber">{{ page.hero.eyebrow }}</p>
