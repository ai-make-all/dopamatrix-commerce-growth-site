...
Nuxt 这边建议用静态生成，Cloudflare Pages 部署时构建命令用 `nuxt generate` / `nuxi generate`，输出目录为 `dist`；Nuxt 官方也明确 Cloudflare Pages 静态生成使用 `nuxt generate`，SSG 会生成可托管的静态目录。([Nuxt][1])
